...
//             <img src="/Logow.png" alt="BuildCrew" className="w-50 h-50 mb-2" />
//              {/* <div className='mb-7'>
...
export const Footer = () => {
  ...
  return (
    <footer 
      className="bg-black text-white pt-20 pb-12 relative overflow-hidden"
      role="contentinfo"
      aria-label="Site footer"
      itemScope 
      itemType="https://schema.org/Organization"
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        ...
        <div className="grid md:grid-cols-12 gap-12 mb-16">
          ...
          <div className="md:col-span-6 lg:col-span-5">
            ...
            <p className="text-gray-300 text-lg leading-relaxed max-w-xl">
              We transform bold ideas into exceptional digital experiences through innovative design and cutting-edge technology.
            </p>
          </div>
            ...
            >
              {navigationItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className="group flex items-center justify-between text-gray-300 hover:text-white transition-all duration-300 py-2"
                  onClick={(e) => handleNavClick(e, item.path, item.name)}
                >
              ...
              {contactInfo.map((info, index) => {
                ...
                const content = (
                  <div className="flex items-start gap-4 group">
                    <div className="mt-1 p-2 rounded-lg bg-white/5 group-hover:bg-white/10 transition-all duration-300">
                      <Icon className="w-4 h-4 text-gray-400 group-hover:text-white transition-colors" />
                    </div>
                    <span className="text-gray-300 group-hover:text-white transition-colors text-base leading-relaxed">
                      {info.content}
                    </span>
                  </div>
                );
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 text-xs mt-2">
              Crafted with precision and passion for exceptional digital experiences.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-6 md:gap-8">
            <Link 
              to="/privacy" 
              className="text-sm text-gray-400 hover:text-white transition-colors border-b border-transparent hover:border-white/30 pb-1"
            >
              Privacy Policy
            </Link>
            <Link 
        ...
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="fixed bottom-8 right-8 p-3 bg-black/80 backdrop-blur-md border border-white/10 rounded-full shadow-2xl transition-all duration-300 hover:bg-black hover:scale-110 hover:border-white/20 z-50"
          aria-label="Scroll back to top"
        >
